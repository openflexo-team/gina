 ...
 * @author sylvain
 */
public class JEditableBoxLayout extends JBoxLayout implements JFIBEditableLayoutManager<JPanel, JComponent, BoxLayoutConstraints> {

	private static final Logger logger = FlexoLogger.getLogger(JFIBEditablePanelView.class.getPackage().getName());

	public JEditableBoxLayout(JFIBEditablePanelView panelView) {
		super(panelView);
	}
	...
	public List<PlaceHolder> makePlaceHolders(final Dimension preferredSize) {
		...
		if (!getComponent().getProtectContent()) {
			...
			panel.setLayout(makeBoxLayout(panel));
			...
			for (int i = 0; i < getComponent().getSubComponents().size(); i++) {
				...
				PlaceHolder newPlaceHolder = new PlaceHolder(getContainerView(), "< item >", placeHolderBounds) {
					...
					public void insertComponent(FIBComponent newComponent, int oldIndex) {
						...
						newComponent.setConstraints(boxConstraints);
	...
	private Rectangle makePlaceHolderBounds(Component component, int deltaX, int deltaY, Dimension preferredSize) {
		...
		if (placeHolderBounds.x + placeHolderBounds.width > getContainerView().getResultingJComponent().getWidth()) {
			...
			placeHolderBounds.x = getContainerView().getResultingJComponent().getWidth() - placeHolderBounds.width;
		}
		if (placeHolderBounds.y + placeHolderBounds.height > getContainerView().getResultingJComponent().getHeight()) {
			...
			placeHolderBounds.y = getContainerView().getResultingJComponent().getHeight() - placeHolderBounds.height;
		}
