...
public abstract class ValidationDialogPanel extends FIBJPanel<ValidationReport> {

	public static Resource VALIDATION_PANEL_FIB = ResourceLocator.locateResource("Fib/ValidationDialog.fib");

	public ValidationDialogPanel(ValidationReport validationReport, FIBLibrary fibLibrary, LocalizedDelegate parentLocalizer) {
	...
	protected ValidationFIBController makeFIBController(FIBComponent fibComponent, LocalizedDelegate parentLocalizer) {
		return new ValidationFIBController(fibComponent, SwingViewFactory.INSTANCE) {
			...
			protected void performSelect(ValidationIssue<?, ?> validationIssue) {
				ValidationDialogPanel.this.performSelect(validationIssue);
			}
		};
	}
