...
public abstract interface FIBContainer extends FIBComponent {
	...
	public static abstract class FIBContainerImpl extends FIBComponentImpl implements FIBContainer {
		 ...
		 * super.updateBindingModel(); if (deserializationPerformed) { for
		 * (FIBComponent child : getSubComponents()) {
		 * child.updateBindingModel(); } } }
		 */

		/*
		 * @Override public Vector<FIBComponent> getSubComponents() { return
		 * subComponents; }
		 */

		/*
		 * public void setSubComponents(Vector<FIBComponent> someComponents) {
		 * FIBPropertyNotification<Vector<FIBComponent>> notification =
		 * requireChange(Parameters.subComponents, someComponents); if
		 * (notification != null) { subComponents = someComponents;
		 * hasChanged(notification); } }
		 */

		/*
		 * @Override public void addToSubComponents(FIBComponent aComponent) {
