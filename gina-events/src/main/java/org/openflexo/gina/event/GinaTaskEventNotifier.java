...
public abstract class GinaTaskEventNotifier extends GinaEventNotifier<GinaTaskEventDescription> {
	...
	public void startThreadStack(GinaTask task) {
		if (this.manager != null) {
			this.manager.startThreadStack(task, this.stackEvents);

			EventDescription desc = manager.getFactory().createTaskEvent(GinaTaskEventDescription.STARTED, task.getURID().getIdentifier());
	...
	public void endTask(GinaTask task) {
		...
		if (gseTask != null) {
			...
			EventDescription desc = manager.getFactory().createTaskEvent(GinaTaskEventDescription.FINISHED, task.getURID().getIdentifier());
