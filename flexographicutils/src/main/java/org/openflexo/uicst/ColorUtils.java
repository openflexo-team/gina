...
public class ColorUtils {
	...
	public static class LAB {
		...
		public double computeDiff(LAB lab) {
			...
			t = 1 - 0.17 * Math.cos(Math.toRadians(hPrime - 30)) + 0.24 * Math.cos(Math.toRadians(2 * hPrime))
					+ 0.32 * Math.cos(Math.toRadians(3 * hPrime + 6)) - 0.2 * Math.cos(Math.toRadians(4 * hPrime - 63));
